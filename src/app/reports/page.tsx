...
export default async function ReportsPage() {
    ...
    // For parent users - only show their child's reports
    if (userRole === 'parent') {
        return (
            <PageContainer
    ...
    // For Admin - show student list + interactive report table
    if (userRole === 'admin') {
        return (
            <PageContainer
            ...
            >
                <div className="space-y-6">
                    ...
                    {/* Existing Reports - now interactive */}
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        <div className="p-4 border-b border-slate-100 bg-slate-50">
                            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide">All Documents</h3>
                            <p className="text-xs text-slate-500 mt-1">Preview, download, or delete reports</p>
                        </div>
                        <ReportTable reports={reports} userRole={userRole} />
                    </div>
                </div>
